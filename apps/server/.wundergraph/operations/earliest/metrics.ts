...
export default createOperation.query({
  ...
  handler: async (ctx) => {
    ...
    if (!arbitrumBlock || !ethereumBlock || !fantomBlock || !polygonBlock || !baseBlock) {
      throw new UpstreamSubgraphError({ message: `${FUNC}: Could not find earliest tokenRecord block for each chain. Arbitrum: ${arbitrumBlock}, Ethereum: ${ethereumBlock}, Fantom: ${fantomBlock}, Polygon: ${polygonBlock}, Base: ${baseBlock}` });
    }
